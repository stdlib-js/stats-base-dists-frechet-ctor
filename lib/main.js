...
function Frechet() {
	...
	if ( arguments.length ) {
		...
		m = arguments[ 2 ];
		if ( !isPositive( alpha ) ) {
			throw new TypeError( format( '10R7d,HT', alpha ) );
		}
		if ( !isPositive( s ) ) {
			throw new TypeError( format( '10R91,Hc', s ) );
		}
		if ( !isNumber( m ) || isnan( m ) ) {
			throw new TypeError( format( '10R8p,HM', m ) );
		}
	} else {
		alpha = 1.0;
		s = 1.0;
	...
	defineProperty( this, 'alpha', {
		...
		'set': function set( value ) {
			if ( !isPositive( value ) ) {
				throw new TypeError( format( '10R8k,HE', value ) );
			}
			alpha = value;
		}
	});
	...
	defineProperty( this, 'm', {
		...
		'set': function set( value ) {
			if ( !isNumber( value ) || isnan( value ) ) {
				throw new TypeError( format( '10R8d,H8', value ) );
			}
			m = value;
		}
	});
