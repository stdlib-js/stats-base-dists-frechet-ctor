...
function Frechet() {
	...
	if ( arguments.length ) {
		...
		m = arguments[ 2 ];
		if ( !isPositive( alpha ) ) {
			throw new TypeError( format( '10R7d', alpha ) );
		}
		if ( !isPositive( s ) ) {
			throw new TypeError( format( '10R91', s ) );
		}
		if ( !isNumber( m ) || isnan( m ) ) {
			throw new TypeError( format( '10R8p', m ) );
		}
	} else {
		alpha = 1.0;
		s = 1.0;
	...
	defineProperty( this, 'alpha', {
		...
		'set': function set( value ) {
			if ( !isPositive( value ) ) {
				throw new TypeError( format( '10R8k', value ) );
			}
			alpha = value;
		}
	});
	...
	defineProperty( this, 'm', {
		...
		'set': function set( value ) {
			if ( !isNumber( value ) || isnan( value ) ) {
				throw new TypeError( format( '10R8d', value ) );
			}
			m = value;
		}
	});
